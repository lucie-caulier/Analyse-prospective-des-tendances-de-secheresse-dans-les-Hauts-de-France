...
calcul_seuils <- function(df, seuil) {
  df %>%
    group_by(code_station, Nom) %>%
    summarise(
      Qx = quantile(Debit, seuil, na.rm = TRUE),
      .groups = "drop"
    )
}
...
#### Fonction pour calculer les VCNx histo de plusieurs stations ####
VCNx_sta_mult_histo <- function (q_jr_totaux_def_histo, jours_glissant) {
  
  map(.x = unique(q_jr_totaux_def_histo$code_station),
      .f = function (x){
        q_jr<-q_jr_totaux_def_histo %>% 
          filter(code_station == x) %>% 
          arrange(date_obs_elab)
        
        vecteur_debits_spe <- q_jr$resultat_obs_elab_spe_l_s_km2
        vecteur_dates <- q_jr$date_obs_elab 
...
###Fonction pour calculer le QMNA5 pour chaque station 

calcul_QMNA5 <- function(df, date = "date", debit = "debit") {
  library(dplyr)
  library(zoo) #pour le lissage glissant 
  
  df <- df %>%
    rename(date = all_of(date), debit = all_of(debit))%>%
    arrange(date) #Trie les lignes du data.frame par date du plus ancien au plus récent, indispensable pour faire le calcul glissant après
  
  
  # Lissage 5 jours glissants
  df <- df %>%
    mutate(debit_7j = zoo::rollmean(debit, 5, fill = NA, align = "right"))
  
  # Débit minimal de chaque année
  qmin_annee <- df %>%
    group_by(annee) %>%
    summarise(Qmin = min(debit_7j, na.rm = TRUE)) %>%
    ungroup()
  
  # Estimation empirique du QMNA5 : quantile de retour 5 ans (~0.20 si on a 20 ans de données)
  n_years <- nrow(qmin_annee)
  proba_retour <- 1 - 1 / 5  # 80% de probabilité d’excès
  qmna5 <- quantile(qmin_annee$Qmin, probs = 1 - proba_retour, type = 6, na.rm = TRUE)
