...
read_drias <- function(file) {
  lines <- readLines(file, warn = FALSE)
  # Trouver l'indice de la première ligne qui ne commence PAS par "#"
  data_start <- which(!grepl("^#", lines))[1]
  if (is.na(data_start)) {
    warning(paste("Pas de données trouvées dans", file))
    return(NULL)
  }
  
  # Lire les données à partir de cette ligne
  df <- tryCatch(
    read.table(file, skip = data_start - 1, sep = ";", header = FALSE,
               col.names = c("Date", "CodeHydro", "IdStation", "X_L93", "Y_L93", "Debit"),
               stringsAsFactors = FALSE),
    error = function(e) {
      warning(paste("Erreur de lecture pour", file, ":", e$message))
      return(NULL)
    }
  )
